...
在会话里点一下 `.plugin` 文件，Margy 会提出把它接收过来。

这不是靠翻译代码做到的，而是因为几乎没有什么需要翻译。exteraGram 里的 Python
和 Margy 里的 Python，是同一个 Telegram 里的同一个 Chaquopy Python；而它们的
`base_plugin`、`client_utils`、`ui.alert` 等等，不过是包着同一套 Java 的普通
Python 外壳，而那套 Java 我们自己的插件也看得见。所以引擎旁边放着同名的模块，
插件文件原样搬过来：变的是打包，外加一段把它们的类启动起来的尾巴。

这段尾巴直接写进源码里，并附带说明——插件仍然可读，也看得出多出来的几行是从
哪来的。

不会有的东西，而且是在安装**之前**就说清楚，而不是事后才发现：

| 什么 | 为什么 |
|---|---|
| `java_subclass` | Python 的桥只继承 Java 接口，不继承类 |
| 从插件发送文件和图片 | Margy 还没有这扇门 |
| 附件菜单 | 这里开了四个菜单，其中没有它 |
| `__requirements__` | 我们不用 pip 装包 |

其余的——它们的元数据、插件生命周期、设置界面、菜单行、发送钩子和请求拦截
——都能一对一地落到我们的门上。
